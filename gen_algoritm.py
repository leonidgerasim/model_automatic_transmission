...
class GenAlgorithm:
    ...
    def get_decision(self):
        ...
        decision.graph()
        print(decision.chrom + [decision.results['time'][-1]])
        df = pd.DataFrame([decision.chrom + [decision.results['time'][-1]]])
        print(df)
    ...
    def print_generations(self):
        ...
        for gen in range(len(self.generations)):
            ...
            for i in range(len(self.generations[gen])):
                m.append(self.generations[gen][i].chrom + [self.generations[gen][i].results['time'][-1]])
        df = pd.DataFrame(m)
