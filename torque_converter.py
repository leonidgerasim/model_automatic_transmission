...
class TorqueConverter:
    """Модель гидротрансформатора с характеристическими картами"""

    def __init__(self, diameter, density=860):
        self.diameter = diameter
        self.density = density
        # Характеристики (примерные значения)
        self.speed_ratio_range = np.linspace(0, 0.95, 10)
        self.torque_ratio = np.array([2.2, 2.0, 1.8, 1.6, 1.4, 1.2, 1.05, 1.0, 1.0, 1.0])
        self.capacity_factor = np.array([0.02, 0.018, 0.016, 0.014, 0.012, 0.01, 0.009, 0.008, 0.007, 0.006])

    def compute_torques(self, engine_rpm, turbine_rpm):
        # Защита от нефизичных значений
        engine_rpm = max(0, engine_rpm)
        turbine_rpm = max(0, turbine_rpm)

        speed_ratio = turbine_rpm / max(1, engine_rpm)  # Защита от деления на 0
        speed_ratio = np.clip(speed_ratio, 0, 0.95)
